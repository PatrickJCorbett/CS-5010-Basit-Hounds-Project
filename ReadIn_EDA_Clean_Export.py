...
if __name__ == "__main__":
    ...
    """
    Index(['Week Ending Date', 'State', 'Observed Number', 
    ...
    #See if we have any missing data for fields 'Week Ending Date', 'State',
    ...
    #We do have some missing values for "Observed Number", 
    #"Upper Bound Threshold""Average Expected Count", "Excess Lower Estimate", 
    #and "Excess Higher Estimate". Take a look at these observations to see if 
    ...
    #For the most part, it turns out missing values are from within the past
    ...
    #May need to drop the rows that have missing values.
    
    
    #See if there are any duplicates by State, Date, and Outcome
    check_dups = excess_deaths[['State','Week Ending Date','Outcome']]
    duplicates = check_dups[
        check_dups.duplicated()].sort_values(by = ['State','Week Ending Date'])
    
    check_why_dups = excess_deaths.sort_values(by = ['State',
                                                     'Week Ending Date',
                                                     'Outcome'])
...
NY = excess_deaths_2020.State.isin(['New York', 'New York City'])
NY_data = excess_deaths_2020.loc[NY, :]
...
NY_rows = excess_deaths_2020.loc[NY, :].index
